...
// ****** Windows
#include <windows.h>
//#define DRV_CALLBACK_TYPE CALLBACK
...
int32_t CanTransmitSet(uint32_t  index, uint16_t  cmd,
  uint32_t  time);
...
void CanSetPnPEventCallback(void (CALLBACK *event)(uint32_t  index, int32_t status));
void CanSetStatusEventCallback(void (CALLBACK *event)
  (uint32_t  index, struct TDeviceStatus *device_status));
void CanSetRxEventCallback(void (CALLBACK *event)(uint32_t  index,
...
typedef int32_t (DRV_CALLBACK_TYPE *TCanTransmitSet)(uint32_t index, uint16_t cmd,
  uint32_t time);
